...
from constants.heroes import HERO_POWER_ACTIVE_SHOP, HERO_POWER_ACTIVE_BOARD, HERO_POWER_ACTIVE_ONLY, TIER_LIST

class Player(Game, Hero, Scanner):

    def __init__(self, name, pool, is_human=False):
        Game.__init__(self)
        self.name = name
        self.pool = pool
...
########### MANAGE ACTIONS ###########
        ...
        if self.is_human:
            ...
            #     return ('swap', first, second, len(self.board))
                ...
                if self.hero.name in HERO_POWER_ACTIVE_SHOP and self.gold >= self.hero.power['cost']:
                    return ('power', randint(1, len(self.shop)))
    ...
    def make(self, action):
        ...
        elif not action and not self.is_bot:
            if self.turn == 14:
                return True
            self.next_turn()
    ...
    def play_minion(self, *args):
        ...
        if isinstance(card, Minion):
            ...
            self.hero.power['do'](self, minion=card, add=True) if self.hero.deathwing else None
            ...
            if card.gold:
                card_discovery = self.get_discovery(tier=self.lvl, pool=self.pool)
    ...
    def sold_minion(self, *args):
        ...
        self.add_to_pool(minion.name, minion.lvl, minion.gold) if minion.name[-2:] != '_t' else None
...
########### DISPLAY STATE ############
    ...
    def display_heroes(self, heroes):
        display = l1 = l2 = l3 = l4 = l5 = l6 = l7 = l8 = l9 = ''
        top = "+{}+".format(" CHOOSES AN HERO ".center(98,'-'))
        print(top.center(152))
        for pos, hero in enumerate(heroes):
            l1 += "+{}+".format(hero.center(17,'-')) +' '*5
            l2 += "|{}|".format(' '*17) + ' '*5
            l3 += "| {}|".format(TIER_LIST[hero]['power']['view'][0].ljust(16)) + ' '*5
            l4 += "| {}|".format(TIER_LIST[hero]['power']['view'][1].ljust(16)) + ' '*5
            l5 += "| {}|".format(TIER_LIST[hero]['power']['view'][2].ljust(16)) + ' '*5
            l6 += "|{}|".format(' '*17) + ' '*5
            l7 += "+{}+".format('-'*17) + ' '*5
            l9 += "{}".format(str(pos+1).center(19)) + ' '*5
